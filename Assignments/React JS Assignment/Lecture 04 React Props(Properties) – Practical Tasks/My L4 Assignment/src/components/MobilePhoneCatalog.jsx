...
function MobilePhoneCatalog(props) {
    const styles = {
        ...
        <>
            ...
            <div style={styles.container}>
                {props.names.map((name, index) => {
                    const mobileimage = props.mobileimages[index];
                    const mobilebrand = props.mobilebrands[index];
                    const mobileprice = props.mobileprices[index];

                    return (
                        <div key={index} style={styles.card}>
                            <div style={styles.imageWrapper}>
                                <img src={mobileimage} alt={name} style={styles.image} />
                            </div>
                            <div>
                                <h4 style={styles.title}>{name}</h4>
                                <h4 style={styles.title}>{mobilebrand}</h4>
                                <p style={styles.price}>{mobileprice}</p>
                            </div>
                        </div>
                    );
                })}
...
export default MobilePhoneCatalog